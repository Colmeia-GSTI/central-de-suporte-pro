...
**Onda 2 — ✅ concluída (2026-07-22).** Toast consolidado em Sonner (54 arquivos, −202 linhas, tsc=0/78 testes) e `asaas-nfse` fatiado (2867→2768 linhas; 5 helpers puros→`logic.ts`, guard `deno test` 18/18 + revisor adversarial; deploy disparado no Lovable). Restam apenas **deferidos de baixa prioridade**: dedups de UI (gestão de ativos; multa/juros) e drops de schema **mantidos** por uso real (ver Tier 5).
