...
| 2026-07-22 | `get_integration_active(text)` (RPC nova) | Segurança (#14): retorna só `is_active` (SEM segredos) para staff, para `useDocSync`/`MonitoringPage` não precisarem de SELECT direto em `integration_settings` (que guarda api_key/bot_token no JSONB). Prepara a restrição do SELECT da tabela a admin/manager/financial. | Segurança |
| 2026-07-22 | DROP `technician_badges`, `_billing_hotfix_backup_pix_contamination`, `_billing_migration_backup_inter_to_asaas`, RPC `calculate_penalties` | Limpeza de schema (Tier 5) — objetos órfãos vazios/sem caller, **verificados** sem FK/view/trigger/caller. **MANTIDOS** por estarem em uso: `not_helpful_count` (trigger de feedback o escreve — a auditoria o marcou "morto" só por não ser exibido), colunas Google Calendar (`sync_event` mantida), enum `billing_reminder` (drop de valor de enum é problemático). `types.ts` fica stale até regenerar via Lovable (0 refs de código → sem impacto). | Higiene / Schema |
