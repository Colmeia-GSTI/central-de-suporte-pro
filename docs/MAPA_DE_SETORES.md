...
- ✅ RESOLVIDO + VERIFICADO (B2): recovery de senha estava DUPLAMENTE quebrado. (1) **Codigo**: `forgot-password` resolvia o e-mail so por `client_contacts.email`→`profiles.email`; como **26/35 clientes tem `client_contacts.email` NULL**, caia em "nao encontrado" e devolvia sucesso generico SEM enviar (UX falso "Email Enviado!"). Corrigido: confia no e-mail digitado validando via `generateLink` (auth.users e a fonte da verdade) + fallback a `auth.users.email`; envia pelo `send-email-resend`. (2) **Deploy**: a versao NO AR era a PRE-B2 (e-mail nativo do Auth, desligado) — descobriu-se que **`git push` NAO redeploya edge functions** (so sincroniza o fonte); deploy feito via `mcp__Lovable__send_message` ao agente do Lovable. **Teste real de entrega OK** (2026-06-29: recovery p/ `faturamento@blendpf.com.br` → `message_logs status=delivered`, id Resend presente). `forgot-password` agora declarado `verify_jwt=false` no config.toml. **Ressalva**: 8 clientes ativos so tem e-mail `.internal` (sem caixa real) → so via reset de admin / cadastro de e-mail real.
...
- [x] Surface oficial de gestao de usuarios: `/settings/users` (UsersTab removido). Acesso admin+manager; acoes sensiveis admin-only no backend.
...
- [x] Recovery consolidado no `send-email-resend` (forgot-password) + resolucao robusta (auth.users) + deploy via send_message. [x] Teste real de entrega: `faturamento@blendpf.com.br` → `message_logs delivered` (2026-06-29).
