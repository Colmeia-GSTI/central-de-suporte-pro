...
- **4.5.1 — Filiais (`client_branches`)**: tabela + UI mínima vinculando ativos/contatos/contratos a filiais do mesmo cliente.
  ...
  - **PR #4 — `branch_id` em CMDB de rede (`doc_vlans`, `doc_vpn`, `doc_firewall_rules`, `doc_access_policies`, `doc_internet_links`, `doc_infrastructure`)** ✅ 2026-04-27 (com hotfix) — coluna `branch_id` (FK nullable para `client_branches`, `ON DELETE SET NULL`) + índice parcial nas 6 tabelas. Sem backfill (todas vazias). RLS preservada. Dropdown "Filial" adicionado nos 2 forms manuais existentes hoje (`DocTableInternetLinks`, `DocSectionInfrastructure`) reutilizando `useClientBranchOptions` do PR #3 (pré-seleção de Sede em criação; edição respeita NULL existente via guarda `data?.id`). Componentes de VLANs/VPN/Firewall/AccessPolicies ainda não existem na UI — coluna fica pronta para a Seção 4.5.2 quando esses forms forem criados. **Hotfix incluído**: alinhamento de 3 nomes de coluna em `DocSectionInfrastructure.tsx` (`general_notes`/`gateway_wan_ip`/`gateway_lan_ip` → schema real), realce visual do badge "Sede" em `ClientBranchesList`, e cleanup de dados de teste em produção do cliente VIZU.
  - **PR #5 — `branch_id` em client_contacts + portal do cliente**: ☐ pendente

> **Nota (2026-05-12) — Refatoração estrutural do portal já executada (não substitui PR #5):** abertura de chamados pelo portal estava quebrada (RLS frouxa + caminho de INSERT direto). Refatoração completa entregue: migration `20260512043108` consolida 3 policies INSERT em 1 + RPC `open_client_portal_ticket` SECURITY DEFINER; novo `ClientTicketForm` com Zod chama a RPC; `ClientPortalPage.tsx` reduzido de 1059 → 200 LOC (split em 5 sub-componentes em `src/pages/client-portal/components/`); `src/lib/phone.ts` centralizado; saldo ~-300 LOC. Validado em produção. PR #5 (adicionar `branch_id` em `client_contacts`) segue pendente — escopo independente. Detalhes no CHANGELOG.

> **Nota (2026-05-19) — DÉBITO ABERTO: Signup público no portal gera contatos órfãos.** Hotfix do dia revelou que 10 usuários haviam criado conta via signup público, recebido role `client` automaticamente, mas ficaram sem vínculo em `client_contacts` (nunca conseguiriam abrir chamado). 7 foram auto-vinculados por domínio do email, 3 ficam pendentes (rede Mad Boards/Madness — cliente ainda não cadastrado). RPC `open_client_portal_ticket` foi hardenizada (`20260519175933`) para dar mensagens específicas, mas a causa estrutural persiste. **Decisão pendente** sobre solução definitiva: (a) bloquear signup público no portal e exigir convite admin (mais seguro, alinha com multi-tenancy futura); (b) auto-vínculo no signup por domínio com confirmação admin (menos atrito mas dependente de domínio único por cliente); (c) painel `/settings/orphan-contacts` listando órfãos para vincular manualmente (curativo, não previne). Detalhes do hotfix no CHANGELOG.
...
- **4.5.7 — Vínculo computador↔chamado**: campo de seleção de ativo no form de novo ticket; popular `tickets.asset_id` automaticamente quando cliente é selecionado. **(Parcialmente antecipado em 2026-04-29 — PR mini-4.5.7):** input de telefone+WhatsApp e dropdown/texto livre de hostname adicionados ao form do PORTAL DO CLIENTE com persistência (4 colunas novas em `tickets` + constraint XOR + policy RLS). Pendente: dashboard "PC teve N chamados nos últimos 30 dias", métricas históricas, e equivalente no form de ticket do STAFF.
