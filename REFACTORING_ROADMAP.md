...
> **Nota (2026-05-19) — DÉBITO RESOLVIDO ✅:** Signup público bloqueado, fluxo de convite admin implementado. Migration `20260519185808` aplicada em produção: tabela `pending_invites`, trigger `enforce_invite_on_signup`, RPCs `accept_invite` / `get_invite_info`, edge functions `invite-user` / `resend-invite` / `revoke-invite`, página `/setup-account`, aba "Convites pendentes" em `/settings/users`. `create-client-user` removido. 10 órfãos antigos continuam funcionando (trigger só age em INSERTs futuros). Pendente: configurar secret `APP_URL` no Lovable Cloud + teste E2E em produção (validar email recebido + redirect). Decisão entre as 3 opções: escolhida **Opção A** (bloquear signup + convite admin) por estar alinhada com multi-tenancy SaaS futura. Detalhes no CHANGELOG.
