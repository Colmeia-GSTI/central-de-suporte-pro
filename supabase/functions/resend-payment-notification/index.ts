...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [emailSettings, emailTemplate, invoiceRes] = await Promise.all([
      getEmailSettings(supabase),
      getEmailTemplate(supabase, "invoice_payment"),
      supabase.from("invoices").select(`
        id, invoice_number, amount, due_date, boleto_barcode, boleto_url, pix_code, payment_method, boleto_status, contract_id,
    ...
    const templateVars: Record<string, string> = {
      ...
      amount: formatCurrencyBRL(invoice.amount),
      due_date: formatDateBR(invoice.due_date),
    ...
    if (channels.includes("email")) {
      ...
      if (!emailTo) {
        ...
        if (emailTemplate) {
          ...
          emailSubject = `Cobrança - Fatura #${invoice.invoice_number} - ${formatCurrencyBRL(invoice.amount)}`;
          const defaultContent = `
            ...
            <div style="background: #f9fafb; padding: 20px; border-radius: 8px; margin: 20px 0;">
              <table style="width: 100%; border-collapse: collapse;">
                <tr><td style="padding: 8px 0; color: #6b7280;">Fatura:</td><td style="padding: 8px 0; font-weight: bold; text-align: right;">#${invoice.invoice_number}</td></tr>
                <tr><td style="padding: 8px 0; color: #6b7280;">Valor:</td><td style="padding: 8px 0; font-weight: bold; text-align: right; color: #059669;">${formatCurrencyBRL(invoice.amount)}</td></tr>
                <tr><td style="padding: 8px 0; color: #6b7280;">Vencimento:</td><td style="padding: 8px 0; font-weight: bold; text-align: right;">${formatDateBR(invoice.due_date)}</td></tr>
              </table>
            </div>
            ${hasBoleto ? `
              <div style="margin: 20px 0;">
                <h3>📋 Boleto Bancário</h3>
                ${boletoSignedUrl ? `<p><a href="${boletoSignedUrl}" style="display: inline-block; padding: 12px 24px; background: ${emailSettings.primaryColor}; color: white; text-decoration: none; border-radius: 6px;">📄 Visualizar Boleto PDF</a></p>` : ""}
        ...
        emailHtml = applyNotificationMessage(emailHtml, notificationMessage, {
          cliente: client.name,
          valor: formatCurrencyBRL(invoice.amount),
        ...
        if (boletoSignedUrl) {
          attachments.push({ filename: `Boleto_${invoice.invoice_number}.pdf`, path: boletoSignedUrl });
        }
        if (nfsePdfSignedUrl) {
          attachments.push({ filename: `NFSe_${invoice.invoice_number}.pdf`, path: nfsePdfSignedUrl });
        }
    ...
    if (channels.includes("whatsapp")) {
      ...
      if (!evolutionSettings || !isWhatsAppActive) {
        ...
        whatsappMessage += `💰 Valor: *${formatCurrencyBRL(invoice.amount)}*\n`;
        whatsappMessage += `📅 Vencimento: *${formatDateBR(invoice.due_date)}*\n\n`;
