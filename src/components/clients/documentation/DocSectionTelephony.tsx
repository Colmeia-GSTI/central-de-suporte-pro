...
const EMPTY: TelephonyData = {
  type: null, provider: null, extensions_count: null, support_phone: null, notes: null,
};
    ...
    <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <Select value={form.type || ""} onValueChange={(v) => setForm({ ...form, type: v })}>
                  ...
                  <SelectContent>{TELEPHONY_TYPES.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div>
                <Label>Fornecedor / Sistema</Label>
                <Input value={form.provider || ""} onChange={(e) => setForm({ ...form, provider: e.target.value })} />
              </div>
              <div>
                <Label>Quantidade de ramais</Label>
                <Input type="number" value={form.extensions_count ?? ""} onChange={(e) => setForm({ ...form, extensions_count: e.target.value ? Number(e.target.value) : null })} />
              </div>
              <div>
                <Label>Telefone suporte fornecedor</Label>
      ...
      <div>
        ...
        <div className="flex flex-col items-center justify-center gap-2 text-muted-foreground py-6">
          ...
          <p className="text-sm">[Tabela de links em construção]</p>
        </div>
      </div>
    </div>
  );
