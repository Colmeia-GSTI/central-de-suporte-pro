...
const EMPTY: SupportHoursData = { business_hours: null, has_oncall: false, oncall_phone: null, sla_critical: null, sla_normal: null, notes: null };
    ...
    <div className="space-y-6">
      <div>
        ...
        {!isEditing ? (
          ...
          <div className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div><Label>Horário de atendimento</Label><Input value={form.business_hours || ""} onChange={(e) => setForm({ ...form, business_hours: e.target.value })} placeholder="Seg–Sex 8h–18h" /></div>
              <div className="flex items-center gap-3 pt-6"><Switch checked={!!form.has_oncall} onCheckedChange={(v) => setForm({ ...form, has_oncall: v, oncall_phone: v ? form.oncall_phone : null })} /><Label>Possui plantão / emergência</Label></div>
              <div className={`transition-all duration-200 ${form.has_oncall ? "opacity-100" : "opacity-0 h-0 overflow-hidden"}`}><Label>Telefone de plantão</Label><Input value={form.oncall_phone || ""} onChange={(e) => setForm({ ...form, oncall_phone: e.target.value })} /></div>
              <div><Label>SLA crítico</Label><Input value={form.sla_critical || ""} onChange={(e) => setForm({ ...form, sla_critical: e.target.value })} placeholder="2 horas" /></div>
              <div><Label>SLA normal</Label><Input value={form.sla_normal || ""} onChange={(e) => setForm({ ...form, sla_normal: e.target.value })} placeholder="8 horas" /></div>
      ...
      <div>
        ...
        <DocTableContacts clientId={clientId} />
      </div>
    </div>
  );
}
